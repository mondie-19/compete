...
export default function VaultTab() {
  ...
  useEffect(() => {
    ...
    const fetchAndSubscribe = async () => {
      ...
      if (data && active) setBalance(data.balance);

      if (active) {
        channel = supabase
          .channel(`vault-updates-${user.id}-${Date.now()}`)
  ...
  const handleTransaction = () => {
    ...
    const minAmount = activeTab === 'withdraw' ? 500 : 1;
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 font-mono">
      <AnimatePresence>
        {status === "success" && (
          <motion.div
          ...
          >
            <div className="text-center">
              ...
              <h2 className="text-2xl font-black italic tracking-tighter uppercase">VAULT <span className="text-compete-purple">SECURED</span></h2>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-6">
          ...
          <div className="bg-[#0F0F16]/60 border border-white/10 backdrop-blur-md rounded-3xl p-6 relative overflow-hidden group">
            ...
            <p className="text-[8px] font-black tracking-[0.3em] text-white/30 mb-1 uppercase">AVAILABLE CREDITS</p>
            <h2 className="text-4xl font-black italic tracking-tighter text-white uppercase">
              Ksh {balance.toLocaleString()}
            </h2>
            <div className="mt-4 flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
              <span className="text-[7px] font-black tracking-widest text-white/40 uppercase">SECURE ESCROW LINKED</span>
            </div>
          </div>

          {/* Action Form */}
          <div className="bg-[#0F0F16]/60 border border-white/10 backdrop-blur-md rounded-3xl p-6 space-y-6">
            <div className="flex bg-white/5 p-1 rounded-2xl border border-white/5">
              <button onClick={() => setActiveTab("deposit")} className={`flex-1 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === 'deposit' ? 'bg-white text-black' : 'text-white/20'}`}>DEPOSIT</button>
              <button onClick={() => setActiveTab("withdraw")} className={`flex-1 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === 'withdraw' ? 'bg-white text-black' : 'text-white/20'}`}>WITHDRAW</button>
            </div>

            <div>
              <div className="relative">
                <span className="absolute left-0 top-1/2 -translate-y-1/2 text-lg font-black italic text-white/10 ml-1">Ksh</span>
                <input
                  ...
                  max="50000"
                  className="w-full bg-transparent border-b border-white/10 py-4 pl-12 text-4xl font-black italic outline-none focus:border-compete-purple transition-all placeholder:text-white/5 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                />
              </div>
              <p className="text-[8px] font-black text-white/20 tracking-widest mt-2 uppercase">
                MIN: {activeTab === 'withdraw' ? '500' : '1'} • MAX: 50,000 KSH
              </p>
            </div>
        ...
        {/* History */}
        <div className="bg-[#0F0F16]/60 border border-white/10 backdrop-blur-md rounded-3xl p-6 flex flex-col h-[400px]">
          <h3 className="text-white font-black italic tracking-[0.25em] text-[8px] lg:text-[9px] mb-6 flex items-center gap-2 opacity-50 uppercase">
            <History size={14} className="text-compete-purple" /> REQUEST HISTORY
          </h3>
          <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
            {withdrawalHistory.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center opacity-10 uppercase">
                <Wallet size={32} className="mb-2" />
                <p className="text-[8px] font-black tracking-widest">NO VAULT ACTIVITY</p>
              </div>
            ) : (
              withdrawalHistory.map((req) => (
                <button 
                  ...
                  className="w-full flex items-center justify-between p-3.5 bg-white/[0.02] rounded-2xl border border-white/10 hover:border-compete-purple/50 hover:bg-white/[0.04] transition-all text-left"
                >
                  <div className="font-mono">
                    <p className="text-xs font-black italic text-white tracking-tighter uppercase">Ksh {req.amount}</p>
                    <p className="text-[7px] font-black text-white/20 tracking-widest uppercase">{new Date(req.created_at).toLocaleDateString()}</p>
                  </div>
                  <div className={`px-3 py-1 rounded-full text-[6px] font-black tracking-widest uppercase ${
