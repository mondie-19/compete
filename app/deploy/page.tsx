...
const PLATFORMS = [
    { id: "PC",     comingSoon: false, icon: <Monitor size={18} className="drop-shadow-[0_0_12px_rgba(255,255,255,0.8)]" />,    color: "hover:border-white hover:shadow-[0_0_20px_rgba(255,255,255,0.6)] hover:text-white" },
    {
        id: "PS",
        comingSoon: false,
        icon: (
    ...
    { id: "MOBILE", comingSoon: false, icon: <Smartphone size={18} className="drop-shadow-[0_0_12px_rgba(155,92,255,0.8)]" />, color: "hover:border-compete-purple hover:shadow-[0_0_20px_rgba(155,92,255,0.6)] hover:text-compete-purple" },
    {
        id: "NINTENDO",
        comingSoon: true,
        icon: (
...
const GAME_PRESETS: Record<string, string[]> = {
    PC:       ["EA FC 26", "PES 26", "PUBG"],
    PS:       ["EA FC 26", "PES 26", "PUBG"],
    XBOX:     ["EA FC 26", "PES 26", "PUBG"],
    MOBILE:   ["CODM", "PUBG", "PES"],
    NINTENDO: [],
};
                            ...
                            <div className="space-y-4 lg:space-y-5 relative z-10">
                                <div className="space-y-2">
                                    <label className="flex items-center gap-2 text-[9px] font-black tracking-widest text-white/40 ml-1 uppercase">
                                        <Gamepad2 size={10} /> SELECT GAME
                                    </label>
                                    <div className="grid grid-cols-3 gap-2">
                                        {(GAME_PRESETS[platform] ?? []).map((game) => (
                                            <button
                                                key={game}
                                                type="button"
                                                onClick={() => setGameName(game)}
                                                className={`py-3 px-2 rounded-xl border text-[9px] font-black uppercase tracking-wider transition-all duration-200 ${
                                                    gameName === game
                                                        ? `${wager.bgClass} ${wager.borderClass} ${wager.colorClass} scale-[1.03] shadow-sm`
                                    ...
                                    <div className="grid grid-cols-5 gap-1.5">
                                        {PLATFORMS.map((p) => (
                                            <button
                                                ...
                                                onClick={() => { if (!p.comingSoon) { setPlatform(p.id); setGameName(""); } }}
                                                className={`relative flex flex-col items-center gap-1.5 py-2 px-1 rounded-xl border transition-all duration-300 ${
                                                    p.comingSoon
                                                        ? 'bg-black/20 border-white/5 text-white/15 cursor-not-allowed opacity-50'
                                                        : platform === p.id
                                            ...
                                            >
                                                ...
                                                {p.comingSoon && (
                                                    <span className="absolute -top-2 left-1/2 -translate-x-1/2 text-[6px] bg-compete-purple/20 text-compete-purple/80 px-1.5 py-0.5 rounded-full whitespace-nowrap font-black tracking-widest border border-compete-purple/20">
                                                        SOON
                                                    </span>
                                                )}
                                            </button>
